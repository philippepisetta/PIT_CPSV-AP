// src/components/encode/steps/EventsStep.tsx
  ...
  return (
    <section className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm space-y-4">
        ...
        <p className="text-sm text-gray-500">
          Sélectionnez les événements généraux de la vie professionnelle ou citoyenne éligibles.
        </p>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
          {lifeEvents.map((event) => (
            <Controller
              key={event}
              name="events.lifeEvents"
              control={control}
              render={({ field }) => {
                const current = field.value || [];
                const checked = current.includes(event);
