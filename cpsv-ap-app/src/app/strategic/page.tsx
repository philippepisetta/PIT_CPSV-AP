...
import EvidencesPage from "@/app/evidences/page";
import { 
  useV2MissionsQuery, 
  useV2RoadmapsQuery, 
...
function StrategicPageContent() {
  const searchParams = useSearchParams();
  const tabParam = searchParams?.get("tab");
  const [activeTab, setActiveTab] = useState<string>("kpis");

  useEffect(() => {
    if (tabParam && ["kpis", "roi", "s3", "evidences"].includes(tabParam)) {
  ...
  const { data: metaRes, isLoading: metaLoading } = useMetaQuery();
  const { data: beneficiariesRes, isLoading: beneficiariesLoading } = useBeneficiariesQuery();
  const { data: activitiesRes, isLoading: activitiesLoading } = useV2ActivitiesListQuery();
  ...
  const loading = missionsLoading || roadmapsLoading || portfoliosLoading || gapLoading || oppsLoading || projectsLoading || outcomesLoading || metaLoading || beneficiariesLoading || activitiesLoading;
  ...
  return (
    ...
    >
      <div className="space-y-6">
        ...
        {/* Navigation Tabs Header (4 Clean Tabs) */}
        <div className="flex bg-glass/25 p-1.5 rounded-2xl border border-muted/20 gap-1 overflow-x-auto scrollbar-thin">
          {[
            { id: "kpis", label: "KPIs d'impact", icon: LineChart },
            { id: "roi", label: "ROI territorial", icon: Euro },
            { id: "s3", label: "Alignement S3 & Gaps", icon: Target },
            { id: "evidences", label: "Registre des Preuves", icon: ClipboardCheck }
          ].map((tab) => {
        ...
        {loading ? (
          <div className="text-center py-20 text-muted font-bold text-xs">
            ...
            <span>Agrégation stratégique en cours...</span>
          </div>
        ) : (
          <div className="space-y-6">
            ...
            {activeTab === "kpis" && (
              <div className="space-y-6">
                        ...
                        <div key={m.id} className="p-4 bg-glass/35 border border-muted/10 rounded-xl space-y-2">
                          ...
                          {m.themes?.map((t: any) => (
                            <div key={t.id} className="p-2.5 bg-glass/20 border border-muted/10 rounded-lg text-[10px] font-bold text-muted flex items-center justify-between">
                              <span>{t.name}</span>
                              <span className="text-[8px] font-black uppercase bg-teal-500/10 text-teal-600 px-1.5 rounded">Thème S3</span>
                            </div>
                          ))}
                        </div>
                      ))}
                ...
                {/* Funnels section */}
                <div className="bg-glass/20 border border-muted/20 rounded-2xl p-6 space-y-6">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-muted/10 pb-4 gap-4">
                    <div>
                      <h3 className="font-extrabold text-text flex items-center gap-2 text-xs uppercase text-muted tracking-wider">
                    ...
                    {selectedFunnel === "edih" && [
                      ...
                      <div key={idx} className="bg-glass/30 border border-muted/15 p-4 rounded-xl space-y-2 relative overflow-hidden">
                        <div className="flex justify-between items-center font-bold">
                          ...
                          {idx > 0 && (
                            <span className="text-[8px] font-black bg-teal-500/10 text-teal-600 px-1 rounded">
            ...
            {/* 2. ROI territorial Tab */}
            {activeTab === "roi" && (
              <div className="space-y-6">
                <div className="bg-glass/25 border border-muted/10 p-5 rounded-2xl space-y-2">
                  <h4 className="text-xs font-black text-text uppercase tracking-wider flex items-center gap-1.5">
                    <Euro className="h-4.5 w-4.5 text-teal-600" />
                    Valorisation de l'Impact Économique Territorial (ROI)
                  </h4>
                  <p className="text-[10px] text-muted font-bold leading-relaxed">
                    Le Retour sur Investissement (ROI) territorial mesure comment chaque euro d'aide publique régionale débloque de la croissance, préserve des emplois ETP et accélère la transition.
                  </p>
                </div>
                  ...
                  {ECOSYSTEMS.map((eco, i) => (
                    <div key={i} className="bg-glass/20 border border-muted/20 rounded-2xl p-5 space-y-4">
                      <div className="flex justify-between items-center border-b border-muted/10 pb-2">
                        ...
                        <span className="text-[9px] font-black px-2 py-0.5 bg-indigo-500/10 text-indigo-650 rounded-full">Pôle</span>
                      </div>
            ...
            {/* 3. Alignement S3 & Gaps Tab */}
            {activeTab === "s3" && (
              <div className="space-y-6">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  {/* Financements */}
                  <div className="bg-glass/20 border border-muted/20 rounded-2xl p-6 space-y-4">
                    <h3 className="font-extrabold text-xs text-text uppercase tracking-wider border-b border-muted/10 pb-2 flex items-center gap-1">
                    ...
                    <div className="space-y-3">
                      {opportunities.map((o: any) => (
                        <div key={o.id} className="p-4 bg-glass/35 border border-muted/10 rounded-xl flex items-center justify-between gap-4">
                          ...
                          <span className="px-2.5 py-0.5 rounded-full text-[9px] font-black uppercase bg-emerald-500/10 text-emerald-600 shrink-0 border border-emerald-500/15">
                            {o.status || "OPEN"}
                          </span>
                        </div>
                      ))}
                  ...
                  <div className="bg-glass/20 border border-muted/20 rounded-2xl p-6 space-y-4">
                    <h3 className="font-extrabold text-xs text-text uppercase tracking-wider border-b border-muted/10 pb-2 flex items-center gap-1">
                      ...
                      Registre des Déséquilibres Régionaux (Gap Analysis)
                    </h3>
                    <div className="space-y-3">
                      {gaps.slice(0, 3).map((fil: any) => (
                        <div key={fil.id} className="p-4 bg-glass/35 border border-muted/10 rounded-xl space-y-2.5">
                          <span className="text-xs font-black text-text block uppercase border-b border-muted/5 pb-1">{fil.name}</span>
                          <div className="space-y-2 font-semibold">
                            {fil.valueChains?.map((vc: any) => (
                              <div key={vc.id} className="space-y-1">
                                <span className="text-[9px] font-black text-muted block uppercase">{vc.name}</span>
                                {vc.segments?.map((seg: any) => {
                                  const hasGaps = seg.gaps?.actors || seg.gaps?.services || seg.gaps?.capabilities || seg.gaps?.funding;
                                  return (
                                    <div key={seg.id} className="p-2 bg-glass/20 border border-muted/10 rounded-lg flex items-center justify-between text-[10px]">
                                      <span className="text-text font-bold">{seg.name}</span>
                                      {hasGaps ? (
                                        <span className="text-rose-500 font-black flex items-center gap-0.5">
                                          <AlertTriangle className="h-3.5 w-3.5 shrink-0" /> Gaps détectés
                                        </span>
                                      ) : (
                ...
                {/* CRUD Strategic Projects & Outcomes + Lineage Visualizer */}
                    ...
                    <div className="bg-glass/20 border border-muted/20 rounded-2xl p-6 space-y-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {projects.map((proj: any) => (
                          <div key={proj.id} className="p-4 bg-glass/35 border border-muted/10 rounded-xl flex flex-col justify-between space-y-3 font-semibold">
                            <div className="space-y-1">
                              <div className="flex justify-between items-start gap-2">
                                <span className="font-bold text-xs text-text leading-tight">{proj.name}</span>
                                <span className="text-[8px] font-black bg-indigo-500/10 text-indigo-600 px-1.5 py-0.2 rounded-full uppercase shrink-0 border border-indigo-500/15">
                                  {proj.status}
                                </span>
                              </div>
                        ...
                        {outcomes.map((out: any) => (
                          <div key={out.id} className="p-3 bg-glass/35 border border-muted/10 rounded-xl flex justify-between items-center gap-4">
                            <div className="space-y-0.5">
                              ...
                              <span className="text-[9px] text-muted block">{out.description || "Aucune description."}</span>
                              {out.publicService && (
                                <span className="text-[8px] text-teal-605 font-bold uppercase block mt-1">
                    ...
                    <div className="bg-glass/20 border border-muted/20 rounded-2xl p-6 space-y-6 sticky top-20">
                      ...
                      {selectedLineageId ? (
                        ...
                        <div className="p-8 text-center bg-glass/10 border border-dashed border-muted/20 rounded-xl text-muted text-xs font-semibold">
                          ...
                          Sélectionnez "Lignage S3" sur un projet pour en tracer la chaîne d'impact.
                        </div>
                      )}
                    </div>
                  </div>
            ...
            {/* 4. Registre des Preuves Tab (Mounts EvidencesPage in readOnly mode) */}
            {activeTab === "evidences" && (
              <div className="space-y-6">
                <EvidencesPage readOnly={true} />
              </div>
            )}
...
function LineageTraceVisualizer({ type, id }: { type: string; id: number }) {
  ...
  const flowSteps = [
    { label: "Programme", key: "programs", color: "bg-rose-500" },
    { label: "Priorité S3", key: "priorities", color: "bg-orange-500" },
    { label: "Initiative", key: "initiatives", color: "bg-amber-500" },
    { label: "Action", key: "actions", color: "bg-emerald-500" },
    { label: "Activité", key: "activities", color: "bg-teal-500" },
    { label: "Défi (Challenge)", key: "challenges", color: "bg-indigo-500" },
    { label: "Parcours (Journey)", key: "journeys", color: "bg-purple-500" },
    { label: "Service public", key: "services", color: "bg-pink-500" },
    { label: "Financement", key: "fundings", color: "bg-sky-500" },
    { label: "Projet R&D", key: "projects", color: "bg-blue-500" },
    { label: "Outcome (Impact)", key: "outcomes", color: "bg-green-500" },
    { label: "Evidence (Preuve)", key: "evidences", color: "bg-violet-500" },
  ];
      ...
      {flowSteps.map((step, idx) => {
        ...
        return (
          <div key={idx} className="flex gap-3 items-start relative pb-4 last:pb-0">
            ...
            {/* Content */}
            <div className="space-y-0.5 bg-glass/25 border border-muted/10 p-2.5 rounded-xl flex-1 min-w-0 font-bold">
...
export default function StrategicPage() {
  return (
    <Suspense fallback={<div className="p-8 text-center text-xs text-muted font-bold">Chargement du cockpit...</div>}>
      <StrategicPageContent />
    </Suspense>
  );
}
