...
function EvidencesPageContent({ readOnly: propReadOnly }: { readOnly?: boolean }) {
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="border-b border-muted/10 text-muted font-black uppercase text-[10px]">
                ...
                {!readOnly && <th className="py-2 text-right">Audit</th>}
              </tr>
            </thead>
            <tbody>
              {evidences.map((evi: any) => (
                <tr key={evi.id} className="border-b border-muted/5 hover:bg-glass/10 transition-colors">
                  ...
                  <td className="py-3 font-mono text-muted">
                    {evi.url ? (
                      <a href={evi.url} target="_blank" rel="noopener noreferrer" className="text-teal-605 hover:underline font-bold">
                        Voir justificatif
                      </a>
                    ) : (
                      "Aucun lien"
                  ...
                  <td className="py-3 font-bold">
                    <span className={`px-2.5 py-0.5 rounded-full text-[9px] font-black uppercase flex items-center gap-1 w-max ${
                      evi.status === "APPROVED" ? "bg-emerald-500/10 text-emerald-600 border border-emerald-500/15" :
                      evi.status === "REJECTED" ? "bg-rose-500/10 text-rose-600 border border-rose-500/15" :
  ...
  // If mounted inside another page layout (like strategic/pilotage), bypass the layout
  if (propReadOnly) {
    return tableContent;
  }
  ...
  return (
    <PITLayout
      category={readOnly ? "WORKSPACE PILOTAGE" : "WORKSPACE ANIMATION"}
      title={readOnly ? "Registre des Preuves (Consultation)" : "Justificatifs & Preuves d’Impact"}
      description={readOnly ? "Consultez le registre des preuves d'impact physique certifiant l'atteinte des KPIs stratégiques territoriaux." : "Consultez et validez les documents justificatifs d'accompagnement et les preuves de résultats territoriaux."}
    ...
    >
      ...
      {readOnly && (
        <div className="p-4 rounded-xl bg-amber-500/10 border border-amber-500/25 text-xs text-amber-800 dark:text-amber-300 font-bold mb-6 flex items-center gap-2">
          <AlertCircle className="h-4.5 w-4.5 text-amber-600" />
          <span>Accès Décideur : Ce registre est présenté en mode consultation seule pour le Cabinet et la Direction Générale.</span>
        </div>
      )}
...
export default function EvidencesPage(props: any) {
  return (
    <Suspense fallback={<div className="p-8 text-center text-xs text-muted font-bold">Chargement du registre...</div>}>
      <EvidencesPageContent {...props} />
    </Suspense>
  );
}
